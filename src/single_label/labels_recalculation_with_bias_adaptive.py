...
# todo: how does it work with other class
...
def calculate_label(
    ...
    grads_ds_orig = np.concatenate([param.grad.detach().clone().cpu().numpy().flatten() for param in model.parameters() if param.requires_grad and len(param) > 0])
    ...
    for hypothetical_labels in labels_matching[:, 1:].T: # first column is ignore
        ...
        for sample_id in range(num_ds_instances):
            ...
            sample_grads = np.concatenate([grads.flatten() for grads in sample_grads]) # only take gradient with respect to weight matrix
