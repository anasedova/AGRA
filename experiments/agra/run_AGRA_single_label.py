...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", help='path to the dataset folder', type=str, default=None)
    ...
    parser.add_argument("--encoding", help='data encoding, set to None for reproduction of ECML results', default=None,
                        choices=['tfidf', 'resnet50'])
    parser.add_argument("--dataset", help='dataset that should be run', default='youtube')
    parser.add_argument("--closs", help='comparison loss', default='CE',
                        choices=['CE', 'F1'])
    parser.add_argument("--weights", help='False: uniform comparison batch sampling, True: class-weighted comparison batch sampling', default='False',
                        choices=['True', 'False'])
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument('--save', type=bool, default=True)
    parser.add_argument("--include_bias", help='True: include the bias gradients in the similarity computation (to reprocude ECML set to False for text datasets)',
    ...
    print('Current combination:', (args.closs, args.weights))
    use_weights = True if args.weights == 'True' else False

    # define storing locations
    # for plots
    storing_loc = os.path.join(os.path.split(os.path.abspath(__file__))[0], 'training_plots', args.dataset, f"comp_loss_{args.closs}_weights_{args.weights}")
    os.makedirs(storing_loc, exist_ok=True)

    # for results
    results_folder = os.path.join(args.output_path, "results", 'agra', args.dataset, f"trl_{'CE'}_cml_{args.closs}_weights_{args.weights}")
    os.makedirs(results_folder, exist_ok=True)
    output_file = open(os.path.join(results_folder, "test_performance.txt"), "w+")

    agra_weights = compute_weights(train_labels_noisy) if args.weights == 'True' else None
