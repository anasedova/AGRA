...
def get_resnet_embedding(train_loader: DataLoader, encoding: str = "resnet50") -> np.ndarray:
    """
...
def load_image_dataset(
    ...
    if os.path.exists(os.path.join(data_path, dataset, f"encoded_{encoding}")):
        ...
        train_embeddings = get_resnet_embedding(train_loader, encoding)
        valid_embeddings = get_resnet_embedding(valid_loader, encoding)
        test_embeddings = get_resnet_embedding(test_loader, encoding)

        with open(os.path.join(data_path, dataset, f"encoded_{encoding}", f"train_embeddings_{encoding}.data"), 'wb') \
                as file:
            pickle.dump(train_embeddings, file)

        with open(os.path.join(data_path, dataset, f"encoded_{encoding}", f"valid_embeddings_{encoding}.data"), 'wb') \
                as file:
            pickle.dump(valid_embeddings, file)

        with open(os.path.join(data_path, dataset, f"encoded_{encoding}", f"test_embeddings_{encoding}.data"), 'wb') \
                as file:
