...
from depr.src import make_plots, eval_performance, collect_statistics, make_legend
from depr.src import calculate_label, F1LossMulti, MaskedBCELoss
from depr.src import MaxEntNetwork
...
def train_grad_match_multilabel(
    ...
    if comparison_loss == "BCE":
        ...
        comparison_criterion = F1LossMulti(ignore_index=None,
                                           reduction=metric_avg)  # ignore_index? -> but should not be necessary
        loss_type = "sum"
    ...
    for epoch in range(num_epochs):
        ...
        auroc = eval_performance(net, dev_data)
    ...
    val_auroc, dev_probs = eval_performance(net_AGRA, dev_data, return_preds=True)

    test_auroc, test_probs = eval_performance(net_AGRA, test_data, return_preds=True)
    ...
    return val_auroc, test_auroc, ignored_0, ignored_1, kept_0, kept_1, dev_probs, test_probs
