...
from src.single_label.labels_recalculation_threshold_wrench import calculate_label
...
class EndClassifierModelWithAGRA(BaseTorchClassModel):
    def __init__(self,
    ...
    def fit(self,
        ...
        if sample_weight is None: # used a bit differently than originally
        ...
        try:
            with trange(n_steps, desc=f"[TRAIN]", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                ...
                for batch in train_dataloader:
                    ...
                    idx = list(WeightedRandomSampler(sample_weight, num_samples=comp_batch_size, replacement=False))
                    ...
                    chosen_labels = calculate_label(batch, weak_train_labels, comp_batch, weak_comp_labels, copy.deepcopy(model), comparison_criterion,
                                                    other_class=other, threshold=agra_threshold, ignore_index=ignore_index)
