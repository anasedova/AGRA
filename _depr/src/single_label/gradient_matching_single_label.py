...
def train_grad_match_with_gold(
    ...
    num_samples = train_X_ds.shape[0]
    num_features = train_X_ds.shape[1]
    ...
    # load the data
    train_data_ds = AGRADatasetWithGold(train_X_ds, train_y_ds, gold_labels=gold_labels, mislabeled=mislabeled)

    # initialize the train dataloader
    train_dataloader_ds_with_subs = DataLoader(train_data_ds, batch_size=int(batch_size), shuffle=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch, (inputs_ds_with_sub, labels_ds_with_sub, gold_labels, mislabeled) in \
            ...
            train_dataloader_orig = DataLoader(
                torch.utils.data.Subset(train_data_ds, idx), batch_size=int(comp_batch_size)
            )
            ...
            chosen_labels, labels_matching = calculate_label(
                inputs_ds_orig,
                labels_ds_orig,
                copy.deepcopy(net),
                optimizer,
            ...
            stats = collect_statistics(
                inputs_ds_with_sub.shape[0], ignore_index, chosen_labels, labels_matching[:, -1],
            ...
            if len(output_ds) > 0:
                loss = criterion(output_ds, torch.tensor(chosen_labels))
